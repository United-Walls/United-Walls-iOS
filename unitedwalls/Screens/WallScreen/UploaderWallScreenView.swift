//
//  UploaderWallScreenView.swift
//  unitedwalls
//
//  Created by Paras KCD on 2023-07-15.
//

import SwiftUI
import SDWebImageSwiftUI

struct UploaderWallScreenView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            if apiManager.selectedUploader != nil {
                ...
                .onChange(of: contentViewViewModel.wallIndex) { index in
                    ...
                    apiManager.loadUploaderFromWallId(wallId: apiManager.selectedUploader!.walls[index]._id)
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) {
                        selectedWall = apiManager.selectedUploader!.walls[index]
                        self.changeOpacity = 1
                    }
                }
                .onAppear {
                    if (contentViewViewModel.wallIndex >= 0 && apiManager.selectedUploader!.walls.count > contentViewViewModel.wallIndex) {
                        selectedWall = apiManager.selectedUploader!.walls[contentViewViewModel.wallIndex]
                        apiManager.loadUploaderFromWallId(wallId: apiManager.selectedUploader!.walls[contentViewViewModel.wallIndex]._id)
                    }
                }
        ...
        .animation(.spring(), value: contentViewViewModel.uploaderWallScreenViewOpened)
    }
}
